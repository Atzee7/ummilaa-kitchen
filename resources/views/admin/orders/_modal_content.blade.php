...
<div class="overflow-y-auto max-h-[60vh] p-6 space-y-5">
    ...
    @if($canCancel)
    <div id="cancel-form-{{ $order->id }}" class="hidden bg-red-50 border border-red-200 rounded-xl p-4">
        ...
        <div class="flex gap-2">
            ...
            <button type="button" onclick="toggleCancelForm({{ $order->id }})"
                class="px-4 py-2 rounded-lg text-xs font-semibold text-gray-500 border border-gray-200 hover:bg-gray-50 transition">
                Batal
            </button>
        </div>
    </div>
    ...
    <div class="bg-red-50 border border-red-100 rounded-xl p-3 flex gap-2">
        ...
        <div>
            ...
            <p class="text-xs text-red-700 leading-relaxed">{{ $order->alasan_pembatalan }}</p>
        </div>
    </div>
    @endif
